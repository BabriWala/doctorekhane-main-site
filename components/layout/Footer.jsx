...
export default function Footer() {
  return <footer className="bg-sky-950 px-4 pt-12 text-sky-100"><div className="mx-auto grid max-w-6xl gap-9 pb-10 sm:grid-cols-2 lg:grid-cols-4">
    ...
    <div><h2 className="font-semibold text-white">দ্রুত লিংক</h2><div className="mt-4 grid gap-2 text-sm">{[["/doctors","ডাক্তার"],["/doctors","মেডিকেল টিম"],["/hospitals","হাসপাতাল"],["/appointment","অ্যাপয়েন্টমেন্ট"],["/blog","স্বাস্থ্য ব্লগ"]].map(([href,label]) => <Link key={`${href}-${label}`} href={href} className="hover:text-white">{label}</Link>)}</div></div>
