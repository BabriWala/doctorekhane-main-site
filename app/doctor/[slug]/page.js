...
export default function DoctorDetailPage({ params }) {
  ...
  return <main className="min-h-screen bg-sky-50/50 pb-16">
    ...
    <div className="mx-auto grid max-w-5xl gap-6 px-4 py-8 lg:grid-cols-[1fr_340px]">
      <div className="space-y-6"><Card><CardHeader><CardTitle>About</CardTitle></CardHeader><CardContent className="space-y-5">{doctor.personalDetails?.about && <p className="leading-7 text-muted-foreground">{doctor.personalDetails.about}</p>}<div className="grid gap-4 sm:grid-cols-2">{doctor.services?.length > 0 && <p className="flex gap-2"><Stethoscope className="h-5 w-5 text-sky-600" />{doctor.services.join(", ")}</p>}{doctor.languages?.length > 0 && <p className="flex gap-2"><Languages className="h-5 w-5 text-sky-600" />{doctor.languages.join(", ")}</p>}{education.length > 0 && <p className="flex gap-2"><Award className="h-5 w-5 text-sky-600" />{education.join(", ")}</p>}<p className="flex gap-2"><CheckCircle2 className="h-5 w-5 text-sky-600" />{doctor.telemedicine ? "Video and in-person consultation" : "In-person consultation"}</p></div>{doctor.conditionsTreated?.length > 0 && <div><h3 className="mb-2 font-semibold">Conditions treated</h3><div className="flex flex-wrap gap-2">{doctor.conditionsTreated.map((item) => <Badge key={item} variant="secondary">{item}</Badge>)}</div></div>}</CardContent></Card>
